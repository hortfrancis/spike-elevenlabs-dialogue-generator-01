...
import {
  listVoices as callListVoices,
  ElevenLabsError,
  type VoiceItem,
} from "../services/elevenlabsService.js";
import { apiError } from "../utils/errors.js";

/** Categories that mean the voice was created by the user (cloned, generated, professional). */
const USER_VOICE_CATEGORIES = new Set(["cloned", "generated", "professional"]);

function isUserVoice(voice: VoiceItem): boolean {
  const cat = voice.category?.toLowerCase();
  return cat !== undefined && USER_VOICE_CATEGORIES.has(cat);
}

export const listVoicesRoute: FastifyPluginAsync = async (app) => {
  app.get<{
    Querystring: { show_legacy?: string };
  }>("/voices/mine", async (request, reply) => {
    ...
    try {
      ...
      const voices = result.voices.filter(isUserVoice);
